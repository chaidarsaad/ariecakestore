    ...
    public function add(Request $request)
    {
        ...
        if($product_check)
        {
            ...
            if($verified_purchase->count() > 0)
            {
                ...
                return redirect()->back()->with('status', "Terimakasih telah memberi rating produk ini");
            }
            else
            {
                return redirect()->back()->with('status', "Anda tidak bisa memberikan rating sebelum membeli produk ini");
            }
        }
        else
        {
